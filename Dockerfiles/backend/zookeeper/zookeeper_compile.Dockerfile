...
RUN zookeeper_module_home=${module_home}/zookeeper && \
    ...
    zookeeper_src_folder=apache-zookeeper-${zookeeper_version} && \
    zookeeper_src_pkg=${zookeeper_src_folder}.tar.gz && \
    zookeeper_src_pkg_url=${apache_repo}/zookeeper/zookeeper-${zookeeper_version}/${zookeeper_src_pkg} && \
    
    mkdir -p ${zookeeper_module_home} && cd ${zookeeper_module_home} && \
    curl -LO ${zookeeper_src_pkg_url} && tar -xzvf ${zookeeper_src_pkg} && rm ${zookeeper_src_pkg} && \
    cd ${zookeeper_src_folder} && source /etc/profile && mvn clean install -DskipTests && mv zookeeper-assembly/target/${zookeeper_pkg} ${zookeeper_module_home}/ && \
    cd .. && rm -r ${zookeeper_src_folder} && tar -xzvf ${zookeeper_pkg} && rm ${zookeeper_pkg} && \
