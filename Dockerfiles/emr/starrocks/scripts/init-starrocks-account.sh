...
# connect local starrocks
connect_with_password=0
while :
do
    ## first: use default no pwd to connect
    starrocks_connect_ret_exec=`mysql -h127.0.0.1 -P${FE_QUERY_PORT} -uroot -e "show backends"`
    starrocks_connect_ret=$?
    if [[ 0 != ${starrocks_connect_ret} ]]; then
        echo "starrocks connect with no password failed"
    else
        echo "starrocks connect with no password success"
        break
    fi
    starrocks_connect_ret_exec=`mysql -h127.0.0.1 -P${FE_QUERY_PORT} -uroot -p${ROOT_PASSWORD} -e "show backends"`
    starrocks_connect_ret=$?
    if [[ 0 != ${starrocks_connect_ret} ]]; then
        echo "starrocks connect with password failed"
    else
        echo "starrocks connect with password success"
        connect_with_password=1
        break
    fi
    
    echo "starrocks has not start, will wait for it"
    sleep 3
done
...
echo "starrocks account init finish!"